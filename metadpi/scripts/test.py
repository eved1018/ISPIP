...
def test_train(test_frame, train_frame, df, feature_cols, annotated_col, rf_params, output_path_dir, model_name):
    df, test_frame, tree = randomforest_test_train(
        test_frame, train_frame, df, feature_cols, annotated_col, rf_params, output_path_dir, model_name)
    df, test_frame = linear_regresion_test_train(
        test_frame, train_frame, df, feature_cols, annotated_col, output_path_dir, model_name)
    df, test_frame = logistic_regresion_test_train(
        test_frame, train_frame, df, feature_cols, annotated_col, output_path_dir, model_name)
...
def randomforest_test_train(test_frame: pd.DataFrame, train_frame: pd.DataFrame, df: pd.DataFrame, feature_cols, annotated_col, rf_params, output_path_dir, model_name) -> tuple:
    ...
    model = RandomForestClassifier(
        n_estimators=trees, random_state=0, bootstrap=False, max_depth=depth, ccp_alpha=ccp)
    model.fit(X, y)
